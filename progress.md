...
| | |
|---|---|
| Branch | `feat/phase-2-recurring` |
| Tests | **523 green**, ~60s |
| Site | `tracker.localhost`, one tracker: `Demo Household` (`TRK-00002`) |
| Shipped | Phase 1 complete · Phase 2: goals, budgets, recurring |
| **Blocking** | **The manual Desk pass — owed since Phase 1, deferred four times** |

### The one thing owed

The manual Desk pass (`docs/manual-test-desk.md`). 523 automated tests say the arithmetic is
...
| A1.1 | Tags | ⬜ | | `Money Tag` + child table; core `Tag Link` absent, so analytics needs a real table |
| A1.2 | Receipts and attachments | ⬜ | | OCR-ready columns filled by nothing until B5 |
| A1.3 | Merchant master | ⬜ | | `Data → Link` + backfill patch |
